...
export const MovieDetail = () => {
  ...
  const image_path = movie.poster_path ? `https://image.tmdb.org/t/p/w500/${movie.poster_path}` : Backup ;

  useEffect(() => {
      async function fetchMovie(){
         const responce = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=dc58eb95082ef4ce69420d6a471c6ceb`);
    ...
    <main>
      <section className='flex justify-around flex-wrap py-5'>
        <div className='max-w-sm'>
            <img className='rounded' src={image_path} alt={movie.title} />
        </div>
